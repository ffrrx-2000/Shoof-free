...
def get_all_mux():
    """الأقسام الـ 19 الثابتة المرسلة مسبقاً"""
    mux_sections = {
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = "🎬 <b>لوحة تحكم Shoof Play</b>\nأهلاً بك! الرجاء اختيار عملية من القائمة:"
...
async def navigate(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "nav_stats":
        await q.edit_message_text("⏳ جاري فحص السعة الدقيقة...")
...
async def upload_video(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if r.status_code == 201:
        ...
        text = f"✅ <b>تم الرفع بنجاح لـ Shoof Play</b>\n\nالفيلم: <b>{movie_name}</b>\n📥 (اضغط للنسخ) Playback ID:\n<code>{p_id}</code>\n\n✨ يمكنك الآن إرسال اسم الفيلم التالي لنفس القسم مباشرة."
...
async def review_assets(update: Update, context: ContextTypes.DEFAULT_TYPE):
    q = update.callback_query; sec = q.data.split("_")[1]
    creds = get_all_mux().get(sec)
    ...
    await q.edit_message_text(text or "القسم فارغ حالياً.", reply_markup=InlineKeyboardMarkup(kb), parse_mode=ParseMode.HTML); return MENU
...
async def admin_home(update: Update, context: ContextTypes.DEFAULT_TYPE):
    kb = [[InlineKeyboardButton("🗑 إدارة الحذف (Asset ID)", callback_data="adm_del_sec")], 
          [InlineKeyboardButton("➕ إضافة قسم", callback_data="adm_add_sec")], [InlineKeyboardButton("🏠 الرئيسية", callback_data="back_home")]]
    if update.callback_query: await update.callback_query.edit_message_text("⚙️ <b>لوحة إدارة Shoof Play</b>", reply_markup=InlineKeyboardMarkup(kb), parse_mode=ParseMode.HTML)
    else: await update.message.reply_text("⚙️ <b>لوحة إدارة Shoof Play</b>", reply_markup=InlineKeyboardMarkup(kb), parse_mode=ParseMode.HTML)
    return ADMIN_HOME
...
async def admin_del_select_sec(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.callback_query.edit_message_text("🗑 اختر القسم للحذف منه:", reply_markup=InlineKeyboardMarkup(kb)); return SELECT_ADM_DEL
...
async def admin_del_list(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    r = requests.get(f"https://api.mux.com/video/v1/assets?limit=15", auth=(creds["id"], creds["secret"]))
    assets = r.json().get("data", [])
    text = f"⚙️ <b>إدارة الحذف للقسم {sec}:</b>\n\n"
    kb = []
    for a in assets:
...
async def delete_asset_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.answer("✅ تم الحذف" if r.status_code == 204 else "❌ فشل الحذف"); return await admin_del_list(update, context)
